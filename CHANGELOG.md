...
- 既存URLがあるのにGREENとなっていた「体の右側だけ不調・怪我（スピリチュアル）」型の誤判定を防止。
- GREENは、既存GSC担当・強近似クエリ・同一テーマ既存URLのいずれも確認されない案件に限定。

## 0.6.4 - 2026-08-16

- Creator候補チェックシートを開くたびに、保存済みの旧判定を再利用せず、現在のDoctor結果とGSC Evidenceから再評価して再生成するよう修正。
- v0.6.3へ更新後も旧GREEN判定がシートに残り続ける回帰不具合を修正。
- Creatorゲートの判定ロジック自体（完全一致=RED、強近似=YELLOW、独立性高=GREEN）はv0.6.3の仕様を維持。
